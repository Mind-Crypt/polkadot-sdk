...
pub(crate) mod tests {
	use super::*;
	use crate::mock::{
		force_new_session, set_next_validators, NextValidators, Guardian, System, Test,
	};
	...
	fn generated_proof_is_good() {
		new_test_ext().execute_with(|| {
			...
			Guardian::on_initialize(2);

			assert!(Historical::historical_root(proof.session).is_some());
			assert!(Guardian::current_index() > proof.session);
